\message{ !name(weka.tex)}\documentclass[12pt,a4paper]{article}
\input{setup/package.tex}
\usepackage{enumerate}
\begin{document}

\message{ !name(es.tex) !offset(-6) }
...
\subsection{JDBC与ElasticSearch}
\par 下面的代码是将Mysql中的表导入到ES中，以建立索引。
\begin{verbatim}
...
\textsl{curl -XGET 'http://localhost:9200/\_nodes/process?pretty=true'}
下面是删除ES上某个index的Restfule命令：
...
\message{ !name(weka.tex) !offset(-161) }
